...
import { ArrowUpRight, ChevronLeft, Receipt } from 'lucide-react';

export default function ExpensePage() {
  const navigate = useNavigate();
  const { expenseTransactions, currentMonth, isLoading, loadFinanceData } = useFinanceStore();
      ...
      <header className="finance-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div>
            ...
            <h1 className="page-title" style={{ fontSize: '1.5rem' }}>Riwayat Pengeluaran</h1>
          </div>
        </div>
      </header>
      ...
      <section className="transactions-section fade-in-up" style={{ animationDelay: '0.2s', marginTop: '1rem' }}>
        {isLoading ? (
          ...
          <div className="tx-list">
            {sortedTransactions.map((t) => (
              <div key={t.id} className="tx-row glass-panel">
                <div className="tx-icon-box bg-danger-light text-danger">
                ...
                <div className="tx-amount-col">
                  <p className="tx-amount text-danger">-{formatCurrency(t.amount)}</p>
          ...
          <div className="empty-box glass-panel">
            <Receipt size={48} strokeWidth={1.5} className="text-muted" opacity={0.5} />
            <p className="empty-text">Belum ada pengeluaran bulan ini</p>
          </div>
        )}
      </section>
    </div>
